...
export const WeatherWidget: React.FC<WeatherWidgetProps> = ({
  ...
  return (
    <div 
      ...
      className="relative w-full rounded-[28px] overflow-hidden bg-[#FDF1DC] dark:bg-[#201B17] border border-[#F6DEBA] dark:border-[#382D24] shadow-sm transition-all"
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden select-none">
        ...
        />
        <div 
          className="absolute top-28 left-44 w-14 h-14 rounded-full border-2 border-[#E89E75]/20 dark:border-[#E89E75]/10"
        />
        <svg 
          className="absolute top-8 left-48 w-8 h-8 text-[#E89E75]/25 dark:text-[#E89E75]/15"
          viewBox="0 0 24 24" 
          fill="none" 
          stroke="currentColor" 
          strokeWidth="2"
      ...
      {/* 2. Main Card Content Grid */}
      <div className="relative z-10 flex flex-col md:flex-row items-stretch justify-between min-h-[200px]">
        
        {/* LEFT COLUMN: Location Name, Full Subtitle, Big Temperature & Live Weather Infos */}
        <div className="flex-1 p-5 sm:p-6 lg:p-7 flex flex-col justify-between z-10 space-y-4">
          
          {/* Location Title & Subtitle with Dropdown Trigger */}
          <div className="relative" ref={dropdownRef}>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                <div>
                  <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-[#9E353B] dark:text-[#E88880] font-odia group-hover:opacity-85 transition-opacity leading-tight">
                    {location.nameOdia}
                  </h2>
                  <div className="text-[11px] font-bold text-[#8B6E5C] dark:text-[#C5A894] tracking-wider uppercase font-sans">
                    {locationHeaderEn}, ODISHA
                  </div>
                </div>
                <div className="w-7 h-7 rounded-full bg-[#F3D7B5] dark:bg-[#34271F] text-[#9E353B] dark:text-[#E88880] flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                  <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${dropdownOpen ? 'rotate-180' : ''}`} />
                </div>
              </button>

              {/* Live Weather Badge / Refresh Button */}
            ...
            {/* Subtitle: Geographical/spiritual tag - Fully visible, responsive text wrap */}
            <div className="flex items-start gap-1.5 mt-2 text-xs font-semibold text-[#8B6E5C] dark:text-[#C5A894] leading-relaxed max-w-xl">
              <MapPin className="w-3.5 h-3.5 text-[#B54047] dark:text-[#E88880] shrink-0 mt-0.5" />
              <p className="font-odia text-xs break-words">
          ...
          {/* Temperature Display & Live Weather Stats */}
          <div className="pt-2">
            <div className="flex items-baseline gap-2">
              <span className="text-5xl sm:text-6xl font-light tracking-tighter text-[#38261F] dark:text-[#F7EFE8] font-sans">
                {isOdia ? toOdiaNumber(tempVal) : tempVal}°
              </span>
              <span className="text-lg font-normal text-[#8B6E5C] dark:text-[#C5A894] font-sans">
                C
              </span>
            </div>

            {/* Weather Infos: Condition, High/Low, Humidity, Wind */}
            <div className="mt-2 flex flex-wrap items-center gap-y-1 gap-x-3 text-xs sm:text-[13px] font-medium text-[#684C3E] dark:text-[#D5BCAC]">
              {/* Weather Condition */}
              <div className="flex items-center gap-1.5 font-semibold text-[#8E282E] dark:text-[#F39A94]">
        ...
        {/* RIGHT COLUMN: Minimal Illustration Art of that Place's Monument, Temple, or Nature */}
        <div className="w-full md:w-[46%] lg:w-[48%] xl:w-[50%] h-44 sm:h-52 md:h-auto min-h-[175px] sm:min-h-[195px] flex items-end justify-end overflow-hidden shrink-0 mt-1 md:mt-0">
